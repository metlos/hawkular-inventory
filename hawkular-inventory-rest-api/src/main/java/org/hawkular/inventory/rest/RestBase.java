...
public class RestBase {
    ...
    protected <T> Response.ResponseBuilder pagedResponse(Response.ResponseBuilder response, UriInfo uriInfo,
        ...
        if (streaming) {
            ...
            try {
                RestApiLogger.LOGGER.info("Fetching data from backend");
                List<?> data = page.toList();
                RestApiLogger.LOGGER.info("Finished fetching data from backend");
                ...
                RestApiLogger.LOGGER.info("Finished building paged response (no data sent to client yet)");
            }
        }
    }
